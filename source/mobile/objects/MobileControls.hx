...
class Config
{
	...
	public function new(saveName:String)
	{
		...
		if (saveName == 'saved-extendControls')
			isExtend = true;
	}
...
class MobileControls extends FlxSpriteGroup
{
	...
	public function new(?customControllerValue:Int, ?CustomMode:String, ?CustomAction:String)
	{
		...
		if (customControllerValue != null)
			mode = getModeFromNumber(customControllerValue);
		...
		switch (vpadMode)
		{
			case 0:
				if (CustomAction != null)
					vpad = new MobilePad(CustomMode, CustomAction);
				else
